...
// Loop the collection
const renderCard = () => {
// Create an html template for each recipe object
		...
		<article data-id="${recipe.id}">
			<header>
				<h2>${recipe.title}</h2>
					...
					<img src="${recipe.picture}" alt="tasty">
					<ul>
						<li>Timing: ${recipe.timing}</li>
						<li>Difficulty: ${recipe.difficulty}</li>
					</ul>
					<button class="info" aria-label="More Info">More Info</button>
	    </article>
`;
...
const openModal = (recipe) => {
	...
	// 	let listString = `<li>${ingredient}</li>`;
	// 	ingredientHTML += listString;
	// })
	const modalHTML = `
	<header>
	<h1>${recipe.title} by <small>${recipe.author}</small></h1>
	</header>
...
innerModal.addEventListener(`click`, openModal);
